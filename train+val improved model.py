...
"""
IFCNN Stage-2 TRAIN + VAL + Live Plots (Loss & Metrics)
======================================================

This script trains our improved IFCNN model with:
1) Adam optimizer (instead of SGD)
2) Two-stage training:
   - Epochs 1..10 : train only the extra residual refinement blocks (conv3_extra) + conv4
   - Epochs 11..  : train conv1 + conv2 + conv3 + conv3_extra + conv4

- Loads baseline weights: snapshots/IFCNN-MAX.pth
- Loss = LAMBDA_MSE*MSE + LAMBDA_PER*PerceptualLoss
- Perceptual net: ResNet101
- Batch size 1 + gradient accumulation
- Poly LR schedule
- After each epoch:
    * train/val loss
    * validate metrics vs GT
...
from model5_deeper import myIFCNN
...
# 1) Paths
# =========================================================
EXPERIMENT_NAME = "improved_model_stage2_adam"
MODEL_TAG = "IFCNN-MAX-improved_model_stage2_adam"
...
UNFREEZE_AFTER_EPOCHS = 10  # train only extra blocks for first 10 epochs
...
# 9) Train / Val loops
# =========================================================
def _freeze_bn_eval(model: nn.Module):
...
def run_one_epoch_train(model, perc_net, loader, optimizer, cur_update, total_updates, base_lr):
    model.train()
    ...
    for inputs, gt, _scene in loader:
        ...
        if do_step or last_batch:
            ...
            cur_update += 1
            lr_now = poly_lr(base_lr, cur_update, total_updates, POLY_POWER)
...
def validate_and_save_outputs(model: nn.Module):
    ...
    for i, scene in enumerate(scenes, start=1):
        try:
            ...
            gt_rgb_u8 = np.array(gt_img, dtype=np.uint8)
...
# =========================================================
# 11) Stage-wise freezing / optimizer creation
# =========================================================
...
def set_train_stage(model: nn.Module, stage: int):
    """
    stage=1: train ONLY conv3_extra + conv4
    stage=2: train conv1 + conv2 + conv3 + conv3_extra + conv4
    ...
    if hasattr(model, "conv3_extra") and model.conv3_extra is not None:
        for blk in model.conv3_extra:
    ...
    print(f"[STAGE] stage={stage} | trainable params: {n_trainable:,}/{n_total:,}")


def make_optimizer_adam(model: nn.Module, lr: float) -> torch.optim.Optimizer:
...
def main():
    ...
    updates_per_epoch = int(math.ceil(len(train_loader) / max(1, ACCUM_STEPS)))
    total_updates = EPOCHS * max(1, updates_per_epoch)
    cur_update = 0


    set_train_stage(model, stage=1)
    optimizer = make_optimizer_adam(model, lr=LR)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if ep == UNFREEZE_AFTER_EPOCHS + 1:
            print(f"\n[STAGE] Switching to STAGE-2 at epoch {ep} (unfreeze conv1/2/3)...")
            set_train_stage(model, stage=2)

            lr_now = poly_lr(LR, cur_update, total_updates, POLY_POWER)
            optimizer = make_optimizer_adam(model, lr=lr_now)
    ...
    print(f"Metrics history CSV: {METRICS_HISTORY_CSV}")
